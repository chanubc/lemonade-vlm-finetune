...
> ⚠️ **읽는 법 주의:** 이 두 문항은 "직전 행동 / 현재 활동 단계"를 묻는 **추론(Reasoning)** 문제다.
> 정답은 지금 보이는 프레임에 **직접 드러나지 않는다**(과거 행동·맥락). 즉 모델이 정답을 맞힌 건
> "이미지에서 답을 봤다"기보다 **학습으로 익힌 행동 순서·맥락 패턴** 덕분이다. 프레임과 정답이
> 겉보기에 안 맞아 보이는 게 정상이다. (반면 `samples/README.md`의 "지금 무슨 행동?" 문제는
> 답이 화면에 보인다.)
